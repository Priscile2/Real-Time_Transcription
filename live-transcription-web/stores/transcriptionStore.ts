...
function handleFileUpload(event: Event) {
  ...
  if (input && input.files && input.files[0]) {
    ...
    // Example: You can read the file in chunks
    const reader = new FileReader();
    reader.onload = (e) => {
      const audioChunk = e.target?.result as ArrayBuffer;
      if (audioChunk) {
        // Send each chunk to the transcription store
        useTranscriptionStore().sendAudioChunk('wsEndpoint', audioChunk);
      }
    };
