  ...
  actions: {
    ...
    async startRecording(): Promise<void> {
      try {
        ...
        const transcriptionStore = useTranscriptionStore();
        const config = useRuntimeConfig();
        const wsEndpoint = config.public.audio.transcriptionWsEndpoint;

        await transcriptionStore.connectWebSocket(wsEndpoint);
        ...
        this.audioWorklet.port.onmessage = async (event) => {
          ...
          if (type === 'chunk') {
            ...
            await transcriptionStore.sendAudioChunk(wsEndpoint, chunk.wavData.buffer as ArrayBuffer);
